...
func (s *Storage) GetProjects(userID string) (projects []entities.Project, err error) {
	if err := s.db.Select(&projects, "SELECT projects.name, username as manager FROM projects LEFT JOIN employees ON projects.manager_id = employees.profile_id WHERE projects.name != ''"); err != nil {
